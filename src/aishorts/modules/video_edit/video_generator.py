    ...
    async def ensure_assets_local(self, reel: Reel):
        ...
        for block in reel.blocks:
            ...
            if voice_filepath and not os.path.exists(voice_filepath):
                if voice_url:
                    print(
                        f"File missing: {voice_filepath}. Re-downloading from {voice_url}"
                    )
                    os.makedirs(os.path.dirname(voice_filepath), exist_ok=True)
                    download_tasks.append(
                        download_from_url(voice_url, full_path=voice_filepath)
                    )
                else:
                    print(
                        f"WARNING: File missing and no URL for voice_filepath: {voice_filepath}"
                    )
            ...
            if lipsync_filepath and not os.path.exists(lipsync_filepath):
                if lipsync_url:
                    print(
                        f"File missing: {lipsync_filepath}. Re-downloading from {lipsync_url}"
                    )
                    os.makedirs(os.path.dirname(lipsync_filepath), exist_ok=True)
                    download_tasks.append(
                        download_from_url(lipsync_url, full_path=lipsync_filepath)
                    )
                else:
                    print(
                        f"WARNING: File missing and no URL for lipsync_filepath: {lipsync_filepath}"
                    )
            ...
            if staticface_filepath and not os.path.exists(staticface_filepath):
                if staticface_url:
                    print(
                        f"File missing: {staticface_filepath}. Re-downloading from {staticface_url}"
                    )
                    os.makedirs(os.path.dirname(staticface_filepath), exist_ok=True)
                    download_tasks.append(
                        download_from_url(staticface_url, full_path=staticface_filepath)
                    )
                else:
                    print(
                        f"WARNING: File missing and no URL for staticface_filepath: {staticface_filepath}"
                    )
            ...
            if song_filepath and not os.path.exists(song_filepath):
                if song_url:
                    print(
                        f"File missing: {song_filepath}. Re-downloading from {song_url}"
                    )
                    os.makedirs(os.path.dirname(song_filepath), exist_ok=True)
                    download_tasks.append(
                        download_from_url(song_url, full_path=song_filepath)
                    )
                else:
                    print(
                        f"WARNING: File missing and no URL for song_filepath: {song_filepath}"
                    )
            ...
            if question_filepath and not os.path.exists(question_filepath):
                if question_url:
                    print(
                        f"File missing: {question_filepath}. Re-downloading from {question_url}"
                    )
                    os.makedirs(os.path.dirname(question_filepath), exist_ok=True)
                    download_tasks.append(
                        download_from_url(question_url, full_path=question_filepath)
                    )
                else:
                    print(
                        f"WARNING: File missing and no URL for question_filepath: {question_filepath}"
                    )
            ...
            for media_id, filepath in assets.media_map.items():
                if filepath and not os.path.exists(filepath):
                    ...
                    if url:
                        ...
                        download_tasks.append(
                            download_from_url(url, full_path=filepath)
                        )
                    else:
                        print(
                            f"WARNING: File missing and no URL for media {media_id}: {filepath}"
                        )
